...
\verb"    explicit "\GOT{rvector}{rvector.rvector (int)}\verb" (int size);"\\
\verb"    "\GOT{rvector}{rvector (std::initializerlist)}\verb" (std::initializer_list<TR> list);"\\
\verb"    "\GOT{rvector}{rvector.rvector (int, TR)}\verb" (int size, TR d);"\\
\verb"    "\GOT{rvector}{rvector.rvector (TR*,int,int)}\verb" (TR* pD, int size, int incr = 1);"\\
\verb"    "\GOT{rvector}{rvector.rvector (const TR*,int,int)}\verb" (const TR* pD, int size, int incr = 1);"\\
...
\verb"    rvector& "\GOT{assign}{rvector.assign (const TR*, int)}\verb" (const TR* p, int incr = 1);"\\
\verb"    rvector& "\GOT{assign}{rvector.assign (int, const TR*, int)}\verb" (int n, const TR* p, int incr = 1);"\\
\verb"    rvector& "\GOT{assign}{rvector.assign (int, const TR*, int, int)}\verb" (int n, const TR* p, int size, int incr);"\\
...
\verb"    rvector& "\GOT{resize}{rvector.resize}\verb" (int new_size) throw (cvmexception);"\\
...
\GOT{rvector::operator -~}{rvector.operator -},
\GOT{rvector}{rvector}.
Example:
\begin{Verbatim}
using namespace cvm;
...
\end{verbatim}
creates an object of type \GOT{rmatrix}{rmatrix}
as  rank-1 update of
a calling vector and a vector \verb"v".
